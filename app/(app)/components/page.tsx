...
import {
  ...
  COMPONENT_SPECS,
  PAGEDEF_STRUCTURE,
} from "../../../src/reference/component-reference-data";
import type { ComponentSpec } from "../../../src/reference/component-reference-data";
import { REGISTRY, getVariantProps } from "../../../src/components/page-def/builder/registry";
import { getAppearanceStyles } from "../../../src/components/page-def/builder/appearance";
import type { PageComponentType, PageDef } from "../../../src/components/page-def/builder/pageDef";
...
const initialPropsJson = COMPONENT_SPECS.reduce(
  (acc, spec) => ({
    ...acc,
    [spec.type]: JSON.stringify(getLivePropsJson(spec, DEFAULT_VALUES[spec.type] ?? ""), null, 2),
  }),
      ...
      <div className="max-w-4xl mx-auto space-y-10">
                ...
                {Object.entries(BASE_PROPS_DOC).map(([prop, spec]) => (
                  <tr key={prop} className="border-b border-slate-100">
                    <td className="py-2 pr-4">
                      <code className="px-1.5 py-0.5 rounded bg-slate-100 text-slate-800">{prop}</code>
        ...
        <section className="space-y-8">
          ...
          {COMPONENT_SPECS.map((spec) => (
            <div key={spec.type} className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
