...
export default function LivePreviewPage() {
  ...
  const initialDef = savedFormDef ?? EXAMPLE_PAGE_DEF;
  ...
  const [showDraftBanner, setShowDraftBanner] = useState(Boolean(savedFormDef));

  useEffect(() => {
    if (!savedFormDef) return;
    setPageDef(savedFormDef);
    setJsonInput(JSON.stringify(savedFormDef, null, 2));
    setValues({});
    setFieldErrors({});
      ...
      <div className="md:w-1/2 space-y-4 min-h-0 flex flex-col">
        ...
        >
          ...
          <ol className="mt-2 list-decimal list-inside space-y-1 text-teal-900/90">
            ...
            <li>After <strong>Save</strong> in the builder, open this screen — it loads your session draft (until you add API persistence).</li>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm flex-1 min-h-0 flex flex-col">
          ...
          {showDraftBanner && (
            <p className="text-sm text-emerald-700 mt-2 rounded-lg bg-emerald-50 border border-emerald-100 px-3 py-2">
              Loaded from your <strong>builder draft</strong> (this session). Edit JSON and click Apply to re-render, or use{" "}
